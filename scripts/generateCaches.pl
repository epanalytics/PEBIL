...
generated by PEBIL binary instrumentation library. 
...
PEBIL/instcode/CacheDescriptions.txt

--out-file <outfile>
====================
The path of the output C header file. An example is given at

PEBIL/instcode/CacheStructures.h

which is generated from the cache description file 
PEBIL/instcode/CacheDescriptions.txt.

For the cache simulation tool in PEBIL to compile, the output
file defaults to CacheStructures.h. Thus, in PEBIL/instcode/ 
...
=========================
The cache simulator in PEBIL gather stride information for
each basic block in addition to the cache simulations. In PEBIL,
